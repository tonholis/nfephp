...
class Damdfe extends CommonNFePHP
{
     ...
     */
    public function __construct($xmlfile='', $sOrientacao='',$sPapel='',$sPathLogo='', $sDestino='I', $sDirPDF='',$fontePDF='',$mododebug=1)
    {
        /*
    ...
    private function __headerMDFe($x,$y,$pag){
        ...
        if (is_file($this->logomarca)){
            ...
            $this->pdf->Image($this->logomarca, 15, 10, 30, 15, 'jpeg');
        } else {
            $x1 = $x;
            $y1 = round($h/3+$y,0);
            $tw = $w;
        ...
        $this->pTextBox($x,$y,$maxW,8,'DAMDFE - Documento Auxiliar de Manifesto Eletronico de Documentos Fiscais',$aFont,'T','C',0,'');
    ...
    private function __bodyMDFe($x,$y){
        ...
        $x1 = $x + 20;
        $this->pTextBox($x,$y,$x1,12);

        $texto = 'Modelo';
        $aFont = array('font'=>$this->fontePadrao,'size'=>8,'style'=>'');
        $this->pTextBox($x,$y,$x1,8,$texto,$aFont,'T','L',0,'',false);

        $texto = $this->mod;
        $aFont = array('font'=>$this->fontePadrao,'size'=>10,'style'=>'');
        $this->pTextBox($x,$y+4,$x1,10,$texto,$aFont,'T','C',0,'',false);

        $x1 += 7;
        $this->pTextBox($x1,$y,$x1,12);

        $texto = 'Série';
        $aFont = array('font'=>$this->fontePadrao,'size'=>8,'style'=>'');
        $this->pTextBox($x1,$y,$x1,8,$texto,$aFont,'T','L',0,'',false);

        $texto = $this->serie;
        $aFont = array('font'=>$this->fontePadrao,'size'=>10,'style'=>'');
        $this->pTextBox($x1,$y+4,$x1,10,$texto,$aFont,'T','C',0,'',false);

        $x1 += 34;

        $this->pTextBox($x1,$y,$x1 - 12,12);

        $texto = 'Número';
        $aFont = array('font'=>$this->fontePadrao,'size'=>8,'style'=>'');
        $this->pTextBox($x1,$y,$x1 - 12,8,$texto,$aFont,'T','L',0,'',false);
        ...
        $x1 += $x2;
        $this->pTextBox($x1,$y,$x2 - 1,10);

        $texto = 'Nº Comprovante';
        $aFont = array('font'=>$this->fontePadrao,'size'=>8,'style'=>'');
        $this->pTextBox($x1,$y,$x2 - 1,8,$texto,$aFont,'T','L',0,'',false);
